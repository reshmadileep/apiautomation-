...
def db_objects_create_backup(local_path_to_create_file, conn):
    ...
    for row in main_cur.execute("select * from dba_objects where object_name='MV_REP_UNMATCHED_INVOICES'"):
        try:
            if row[5] != 'MATERIALIZED VIEW':
                inner_cur.execute(
                    "select dbms_metadata.get_ddl('" + row[5] + "','MV_REP_UNMATCHED_INVOICES') from dual")
                for result in inner_cur:
                    create_temp_file_with_query(local_path_to_create_file, result[0].read(),
                                                'Object_Type_' + row[5] + ".sql")
